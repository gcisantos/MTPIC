...
$(function () {
    ...
    $("#formaBusca").change(function () {
        switch ($("#formaBusca").val()) {
            case "Texto":
                $("#campos").html("<br><label for='texto'>Enter the text you want to process below:</label>" +
        ...
        if (processa) {
            ...
            $.ajax({
                ...
                if (data.erro == "0") {
                    ...
                    alert("Unable to process inserted text, try another way.");
                }
    ...
    function duploClickNode(params) {
        ...
        window.open("https://en.wikipedia.org/wiki/" + encodeURIComponent(items[0].label));
    }
